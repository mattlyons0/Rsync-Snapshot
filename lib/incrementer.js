...
let linkDest = undefined;
let tempDest = undefined;
...
function prepareForBackup(){ //Remove .incomplete folders, create .incomplete folder for this backup, find latest snapshot
  return new Promise((resolve, reject) => {
    ...
    bashCommand+= `find . -maxdepth 1 -type d -name '*.incomplete' | xargs rm -r;`; //Cleanup old incomplete backups
...
function finalizeBackup(){ //Move backup to folder without .incomplete
  return new Promise((resolve, reject) => {
    let finalDest = tempDest.substring(0,tempDest.length-('.incomplete'.length));
