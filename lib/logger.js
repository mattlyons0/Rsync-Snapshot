...
let outputFile = '';
let firstLine = true;
let tempDest, linkDest;

function setLogFormat(format) {
  logFormat = format;
}

async function setOutputFile(filepath){
  if(!filepath)
    return true; //We don't need an output to succeed

  if(!path.isAbsolute(filepath)){ //Convert relative path to absolute
    filepath = path.resolve(filepath);
  }

  try { //Check file path is valid and writable
    await fs.access(path.dirname(filepath), fs.constants.W_OK); //Will throw exception if can't write
    outputFile = filepath;
    return true;
  } catch(e){
    console.error('Invalid Output Path', e);
    return false;
  }
}

function setDestinations(temp, link){
  tempDest = temp;
  linkDest = link;
}
...
function processFinalized(destFile){
  fileAppend(`\n- Backup Complete - ${destFile}\n`);
}
