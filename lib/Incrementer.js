...
class Incrementer{
  ...
  deleteOldSnapshots(){
    return new Promise((resolve, reject) => {
      ...
      debug('Executing on server: '+bashCommand);
  ...
  prepareForBackup(){ //Remove .incomplete folders, create .incomplete folder for this backup, find latest snapshot
    return new Promise(async (resolve, reject) => {
      ...
      let bashCommand = `mkdir '${this.escapeQuotes(this.tempDest)}';`; //Make Temp Directory Path
      ...
      bashCommand+= `INCOMPLETE_LS=$(ls $(echo "$INCOMPLETE_BACKUPS" | head -1));`; //Check if there are files in the latest incomplete folder
      //If there are incomplete backups find most recent and move into current backup dir (to preserve progress)
      bashCommand+= `if [ ! -z "$INCOMPLETE_BACKUPS" ] && [ ! -z "$INCOMPLETE_LS" ]; then ${mv} $(echo "$INCOMPLETE_BACKUPS" | head -1)/* ${this.escapeQuotes(this.tempDest)}; fi;`;
  ...
  finalizeBackup(){ //Move backup to folder without .incomplete
    return new Promise((resolve, reject) => {
      ...
      bashCommand+= `rm -f latest; ln -s '${this.escapeQuotes(this.finalDest)}' latest;`; //Symlink latest to latest backup
      ...
      try {
        ...
        proc.on('exit', (code) => {
          if (code === 0) {
            ...
            let err = new Error('Failed to exit with code 0');
            delete err.stack;
            reject(err);
          }
        });
